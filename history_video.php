...
if($session_id == "")
{
   ...
   $history_sql = mysqli_query($connect,"SELECT distinct (date(`timestamp`)) AS date FROM `course_view_history` WHERE `user` = '$session_id' order by `date` DESC");     
   $row10 = mysqli_fetch_all($history_sql);      
?>
<style>
.cat_stick
{
...
video,.video{
    ...
    <section class="gen-section-padding-3 grid_video" style="margin-top:50px;">
        ...
        <div class="container container-2">
            ...
            <a href="clear_history.php" style="float:right;    background-color: var(--primary-color);color: white !important;    padding: 5px 25px;border-radius: 9px;" onclick="return confirm('Are you sure you want to Clear Watch History?')">Clear Watch History</i></a>            <br>
                     <div class="gen-style-2 videoitems">        
                     
                          <?php
                          foreach($row10 as $date_row)
                          {
                           $date = $date_row[0];

                           echo "<span style='color:black'>".date('d M Y ', strtotime($date))."</span>";
                           $history_sql1 = mysqli_query($connect,"SELECT `course_code` FROM `course_view_history` WHERE date(`timestamp`) = '$date' order by `id` DESC");     
                           while($row11 = mysqli_fetch_assoc($history_sql1))
                           {
                              $code_1[] = $row11['course_code'];
                           }             
                           $code_all = array_unique($code_1);
                           foreach($code_all as $course_code)
                           {
                           $course_sql = mysqli_query($connect,"SELECT * FROM `courses` WHERE `course_code` ='$course_code' AND `status` = '1'");    
            ...
            {
                echo "<div class='gen-style-2 videoitems' style='text-align:center;color:#1261a0'>Watch History Not Available.</div>";
            }
            ?>
